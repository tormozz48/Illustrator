 ...
 */
export async function handleGenerateStyleBible(
  job: Job<GenerateStyleBiblePayload>
) {
  ...
  try {
    ...
    const styleBible = await callGroq(
      STYLE_BIBLE_PROMPT(bookTitle, fullText),
      (data) => StyleBibleResponseSchema.parse(data)
    );
    ...
    await orchestrator.markBookFailed(
      bookId,
      `Style bible generation failed: ${error}`
    );
    throw error;
  }
}
