...
export function sliceChapters(rawText: string, boundaries: ChapterBoundary[]): RawChapter[] {
  ...
  for (let i = 0; i < boundaries.length; i++) {
    ...
    if (content.length === 0) {
      logger.warn(
        `sliceChapters: chapter ${boundary.number} ("${boundary.title}") produced empty content — skipping`
      );
      continue;
    }
